class Contact < ActiveRecord::Base
  ...
  def self.import_contact(user, name, email, provider, picture_url)
      identity = Identity.where(:email => email, :provider_id => provider).first
      if identity.nil?
        identity = Identity.new(:provider_id => provider, :name => name, :email => email, :picture_url => picture_url) 
        identity.confirmed_at = Time.now
        identity.save
      end

      newcontact = Contact.find_or_create_by_user_id_and_email(:user_id => user.id, :name => name, :email => email, :identity_id => identity.id)
      newcontact.save
    newcontact
  end
